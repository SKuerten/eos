...
class VB(object):
    ...
    def run(self):
        self.vb.run(verbose=True, prune=0.5 * self.vb.N / self.vb.K)

        hdf5_io.save_mixture(self.args.output, hdf5_directory, self.vb.make_mixture())
        hdf5_io.save_vb_hyperparameters(self.args.output, hdf5_directory, self.vb)
