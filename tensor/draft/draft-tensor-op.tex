...
$\wilson{9,\,9',\,10,\,10'}$. The experimental inputs of all observables
entering our fits is listed in \reftab{tab:observables} together with input for
...
the enhanced sensitivity of both observables to those couplings compared to
...
For each coupling we vary in a fit, we remain as general as possible, treat it
...
interference effects with new physics in vector couplings. Finally, we conclude
...
In a scenario with only tensor complex-valued couplings $\wilson{T,\, T5}$, the
...
values~\cite{Horgan:2013pva}. Since tensor couplings contribute constructively
...
We also perform a fit with non-zero $\wilson[NP]{9,\,10}$ \fred{Is this
  $\wilson[NP]{}$ ever introduced?} in order to assess the robustness of the
bounds with respect to interference. Note that $\wilson{7,7'}$ appears in linear
combinations with $\wilson{9,9'}$ such that its interference with tensor and
scalar couplings is captured implicitly by allowing new physics in
$\wilson{9,9'}$. Thus we fix $\wilson{7,7'}$ to the SM value without loss of
...
which we attribute to cumulative effect of $\wilson{S,S',P,P'} \ne 0$ in
...
coordinates but never in even the 95\% region in polar coordinates. This is a
consequence of the general \emph{concentration of measure}. Another way to look
at it is to transform the uniform prior density from cartesian to polar
coordinates. For the example of the single Wilson coefficient, say $\wilson{T}$,
the transformed density is proportional to the determinant of the Jacobian which
is $|\wilson{T}|$. In words, the uniform prior on $\mbox{Re}(\wilson{T})$ and
$\mbox{Im}(\wilson{T})$ favors larger values of $|\wilson{T}|$ even though we
consider it consensus in the community that smaller rather than larger values
are held reasonable because in the SM, $\wilson{T} = 0$. We suggest therefore
that the default prior be changed in the future.
...
\begin{figure*}
  ...
  \end{center}
  \caption{ The constraints on complex-valued $\wilson{S,\,S',\,P,\,P'}$ from
    only $F_H^\mu$~[grey dotted, left panel], only $\overline{\cal B}(B_s \to
    \bar\mu\mu)$ [blue dashed, center panel], and the combination with all other
    data in \reftab{tab:observables} as well as non-zero
    $\wilson[]{9,\,9',\,10,\, 10'}$ [red solid, right panel] at 68\% (darker)
    and 95\% (lighter) probability. The constraints on Re$(\wilson{P}Ê\pm
    \wilson{P'})$ are identical to Re$(\wilson{S}Ê\pm \wilson{S'})$, apart from
    a small translation of the contours by $(+0.2,\, +0.15)$. The SM prediction is
...
kinematic suppression---see \refeq{eq:FH-dependence}---as the sum $(\wilson{i} +
\wilson{i'})$ whereas in the branching ratio $\overline{\cal B}(B_s\to
\bar\ell\ell)$ they appear as the difference $(\wilson{i} - \wilson{i'}), i =
S,\, P$.  Since the existing measurement of $F_H^\mu$ constrains the sum, the
combination of $F_H^\mu$ and $\overline{\cal B}(B_s \to \bar\mu\mu)$ allows
us---for the first time---to bound the real and imaginary parts of all four
couplings.  The corresponding 2D-marginalized regions in the
$\mbox{Re}(\wilson{i} \pm \wilson{i'})$ ($i = S, P$) planes are shown in
\reffig{fig:scalar}. The corresponding plots for $\mbox{Im}(\wilson{i} \pm
\wilson{i'})$ are very similar to those shown and thus omitted. These bounds do
not change when including all other data in \reftab{tab:observables}, since the
$A_{\rm FB}^\mu(B\to K\bar\mu\mu)$ requires interference of scalar with tensor
couplings and other observables are not very sensitive to scalar
couplings. Quantitatively, the constraint from $\overline{\cal B} (B_s \to
\bar\mu\mu)$ on $(\wilson{i} - \wilson{i'})$ is about a factor four to five
...
In the following we consider a model-independent scenario as given by the SM but
augmented by dimension six operators that respect the SM gauge group and are
composed of SM fields only. Such a scenario becomes more and more viable for two
reasons. The first is the discovery of a scalar resonance at the LHC in
agreement with all requirements of the Higgs particle in the SM. The second are
the steadily rising lower bounds on the mass of new particles reported by ATLAS
and CMS in various more or less specific models.

A non-redundant set of dimension-six operators of this effective theory (SM-EFT)
was given in \cite{Grzadkowski:2010es} and the matching to the effective theory
of $\Delta B = 1$ decays \refeq{eq:Heff} at the scale $\mu \sim m_W$ of the
order of the $W$-boson mass is presented in~\cite{D'Ambrosio:2002ex} for vector
couplings $\wilson{7,\,9,\,10}$. The matching of tensor \refeq{eq:tensor:ops}
and scalar \refeq{eq:scalar:ops} operators \cite{Alonso:2014csa} shows that SM
gauge groups in conjunction with the linear representation \todo{Ask Martin,
  Oscar how to correctly phrase this} impose the relations
\begin{align}
...
scalar couplings $\wilson{S,\, S'}$ arise that scale as $\sim v^2/\Lambda_{\rm
  NP} \ll 1$ where $v^2$ denotes the scale of electroweak symmetry breaking and
$\Lambda_{\rm NP}$ is the high-energy scale at which new physics generates these
operators.
...
first fit, we set $\wilson[NP]{10, 10'} = 0$ and include all constraints on $B
\to K \mu \bar{\mu}$ and $B_s \to \mu \bar{\mu}$. In the second fit, we allow
$\wilson[NP]{10, 10'} \ne 0$ and further include all $B \to K^*\mu \bar{\mu}$
...
\begin{figure*}
  \begin{center}
    ...
    \caption{ 68\% and 95\% contours of the 2D-marginalized distributions of
      ...
      non-zero $\mbox{Re, Im} (\wilson[NP]{10,\,10'})$.  For comparison, we
      superimpose the corresponding contours from using only $\overline{\cal
        B}(B_s\to \bar\mu\mu)$ and all $B \to K \mu \bar{\mu}$ constraints
...
bosons.  For example choosing a non-linear representation of the scalar sector
...
\begin{acknowledgements}
  ...
  $B\to K^*$ form factors \cite{Straub:2015ica}.  C.B. was supported by the ERC
