...
on the Wilson coefficients: $\wilson[\ell]{i} \to \wilson[]{i}$. In \refsec{sec:results}
...
available for this source of SM background\footnote{The leading QED corrections
  to $B\to X_s\bar\ell\ell$ \cite{Huber:2015sra} \todo{(update ref. with published)} might provide an estimate of
...
\begin{figure*}
  ...
  \caption{ The sensitivity to the tensor coupling $\mbox{Re}(\wilson{T})$ of
    \christoph{$F_H^\mu$} (first row) and \christoph{${\cal B}_\mu$} (second row) 
    in $B^+ \to K^+ \bar\mu\mu$
...
In the following we consider a model-independent scenario \christoph{consisting
