...
\bar\mu^+\mu^-$ that is well known to constitute a valuable probe of
the standard model (SM) and to provide constraints on its extensions
\fred{Here we need some references}.
...
\end{align}
 has been recently updated with 3 fb$^{-1}$
...
factors for vector and dipole
couplings present in the SM, thereby enhancing their sensitivity to tensor and
scalar couplings \cite{Bobeth:2007dw, Bobeth:2012vn}. A similar enhancement of
scalar couplings compared to helicity-suppressed vector couplings of the SM
...
scalar and tensor couplings in this decay mode. In the future also certain
...
non-vanishing tensor or scalar contributions are enhanced unless the
...
$F_H^\ell$ is very sensitive to tensor couplings whereas $A_{\rm FB}^\ell$ to
the interference of tensor and scalar couplings.
...
\begin{figure*}
  ...
  \caption{ The sensitivity to the tensor coupling
      ...
      is available.
      \todo{Perhaps restrict plots to range $C_T \in [-0.7, +0.7]$
            or even $\in [-0.5, +0.5]$?}}
  \label{fig:FHvsCT}
\end{figure*}

There are also angular observables $J_i$ in $B\to K^*(\to K\pi)\,
\bar\ell\ell$ with the same properties, i.e. tensor and scalar contributions
are kinematically enhanced by a factor $\sqrt{q^2}/m_\ell$ over vector ones
...
To illustrate the sensitivity of $F_H^\ell$ to tensor couplings, we compare
it in \reffig{fig:FHvsCT} to the one of the branching ratios of $B\to K^{(*)}
\bar\ell\ell$ \todo{$\ell = \mu$}, integrated over low and high $q^2$. The details of the numerical
...
the sensitivity to tensor couplings is highest for $F_H^\ell$ at high $q^2$,
...
and has some impact on the constraints on tensor couplings as will be
...
tensor couplings. Moreover, the dependence on vector couplings is such that
...
Eventually we also explore the behaviour of derived constraints with respect to
destructive interference with vector couplings $\wilson{9,\,9',\,10,\,10'}$.
For this purpose we include also the branching ratio, the lepton forward-backward
asymmetry and the rate CP asymmetry of $B\to K^* \bar\mu\mu$ as they provide
additional constraints on the real and imaginary parts of vector couplings.
...
\reftab{tab:observables}, mainly due to the sensitivity of the branching ratio of $B\to K^{*}
\bar\mu\mu$ to tensor couplings (see also \reffig{fig:FHvsCT}). The latter
stronger bounds are driven by the new lattice
results of $B\to K^*$ form factors that yield too high predictions compared
to the measurements~\cite{Horgan:2013pva}. Since tensor couplings contribute
constructively to ${\cal B}(B\to K^{*}\bar\mu\mu)$ large values are stronger
constrained. In this scenario, the measurements of $A_{\rm FB}^\mu(B^+\to K^+\bar\mu\mu)$
does not pose constraints due to vanishing scalar couplings.

Eventually, we perform a fit with non-zero complex-valued $\wilson{9,\,10}$
by allowing uniform priors for $\mbox{Re, Im}(\wilson{i}) \in[-7,\, 7]$,
($i = 9,10$) in order to access the stability of the bounds
against destructive interference. In this case, $F_H^\mu$ by itself still provides
bounds on $|\wilson{T,\,T5}|$ that are
...
in $\wilson{9,\,10}$ become reduced and almost the same constraints on $\wilson{T,\,T5}$
are recovered, as shown in \reftab{tab:cTT5:1-dimCLs}.
...
manner $(\wilson{i} + \wilson{i'})$ to the branching ratio
$\overline{\cal B}(B_s\to \bar\ell\ell)$, which constrains only $(\wilson{i} - \wilson{i'})$,
where $i = S,\, P$.
Most notably, we find that the current measurement of $F_H^\mu$ does constrain
the orthogonal $(\wilson{i} + \wilson{i'})$, such that the combination of $F_H^\mu$ and
$\overline{\cal B}(B_s \to \bar\mu\mu)$ allows to bound for the first time the moduli of
all four couplings.

The according four-parameter fit for complex-valued scalar couplings to the data of
$\overline{\cal B}(B_s \to \bar\mu\mu)$ and $F_H^\mu$ results in the (2D-marginalized)
regions in the $\mbox{Re}(\wilson{i} \pm \wilson{i'})$ ($i = S, P$)  planes shown in
\reffig{fig:scalar} and similarly for $\mbox{Im}(\wilson{i} \pm \wilson{i'})$. These
bounds do not change when including all other data in \reftab{tab:observables}, since
the $A_{\rm FB}^\mu(B\to K\bar\mu\mu)$ requires interference of scalar with tensor
couplings and other observables are not very sensitive to scalar
couplings. Quantitatively, the constraint from $\overline{\cal B}
(B_s \to \bar\mu\mu)$ on $(\wilson{i} - \wilson{i'})$ is about \eos{a factor four to five}
stronger than the one of $F_H^\mu$ on $(\wilson{i} + \wilson{i'})$.

Interference terms of $\wilson{P,\,P'}$ with vector couplings might weaken
these bounds. Most important for $\overline{\cal B}(B_s \to \bar\mu\mu)$ are
$(\wilson{10} - \wilson{10'})$ --- compare \refeq{eq:Br-Bsmumu:time0} --- and
for $F_H^\ell$ similarly $(\wilson{10} + \wilson{10'})$ \cite{Bobeth:2007dw},
which are suppressed by factors $m_\mu/M_{B_s}$ and $m_\mu/\sqrt{q^2}$,
...
their combination with all the other observables in \reftab{tab:observables},
even allowing new physics in vector couplings
$\wilson{9,\ 9',\, 10,\, 10'}$ in \reftab{tab:cSP:1-dimCLs}, utilising
uniform priors for $\mbox{Re, Im}(\wilson{i}) \in[-7,\, 7]$.
Neither $F_H^\ell$ nor $\overline{\cal B}(B_s \to \bar\mu\mu)$
...
destructive interference with vector couplings. In the general case
