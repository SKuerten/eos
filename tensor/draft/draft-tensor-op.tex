...
\thankstext[$\star$]{t1}{\christoph{Preprint: EOS-2015-03,} FLAVOUR(267104)-ERC-107}
...
\begin{abstract}
  ...
  combination with \christoph{branching ratio of} $B_s\to \bar\mu\mu$ to derive the strongest model-independent
  bounds on tensor and scalar effective couplings to date. The measurement of
  $F_H^\mu$ \christoph{provides complementary constraints} to that of the branching ratio of $B_s\to
...
during \christoph{Run} I at the Large Hadron Collider (LHC), we now have access
to rather large samples of rare \christoph{$B$-meson} decays with branching ratios
...
Both \christoph{angular} observables, $F_H^\ell$ and $A_{\rm FB}^\ell$, exhibit strong suppression
...
\bar\ell\ell$ from LHCb \cite{Aaij:2013qta} \christoph{did not yet} allow to perform a full
...
\begin{aligned}
  {\cal L}_{\rm eff}  =
   \frac{4 G_F}{\sqrt{2}} \,\frac{\alE}{4 \pi}\, & V_{tb}^{} V_{ts}^\ast
      \, \Big[ \christoph{C_7(\mu_b) \Op_7 + C_{7'}(\mu_b) \Op_{7'}}
 \\ & +
       \sum_{\ell = e,\, \mu,\, \tau} \sum_i  \wilson[\ell]{i}(\mu_b)
       \christoph{\Op_i^\ell} \Big] + \text{h.c.}
   \christoph{\,,}
\end{aligned}
\end{equation}
the most general dimension-six flavor-changing operators \christoph{$\Op_i^{(\ell)}$} mediating $b\to
...
\begin{equation}
  \label{eq:scalar:ops}
\begin{aligned}
  \christoph{\Op_{S(S')}^\ell} &
  = \[\bar{s} P_{R(L)} b\]\!\[\bar{\ell} \ell\]\,,
\\[0.1cm]
  \christoph{\Op_{P(P')}^\ell} &
...
\\[0.1cm]
  \christoph{\Op_{T5}^\ell} &
  = \[\bar{s} \sigma_{\mu\nu} b\]\!\[\bar{\ell} \sigma^{\mu\nu} \gamma_5 \ell\]\,,
\end{aligned}
\end{equation}
where the notation $\christoph{\Op_{T5}^\ell} = i/2\, \varepsilon^{\mu\nu\alpha\beta}
...
\christoph{m_b m_\ell/m_W^2}$, and $\wilson[\ell, SM]{9',10',T,T5} = 0$.
%Beyond the SM,
...
\christoph{For the rest of this article we will suppress the lepton-flavor index
on the Wilson coefficients: $\wilson[\ell]{i} \to \wilson[]{i}$ and 
operators $\Op_i^\ell \to \Op_i$. In \refsec{sec:results}
we exploit data with $\ell =\mu$ only, hence all derived constraints apply in
principle only to the muonic case, but can be carried over to other lepton
species $\ell = e,\, \tau$ in cases of NP models that do not violate lepton
flavor. In general the Wilson coefficients are decomposed into SM and NP
contributions $\wilson[]{i} = \wilson[SM]{i} + \wilson[NP]{i}$, but often we
will use $\wilson[]{i}$ for Wilson coefficients with zero (or suppressed) SM 
contributions synonymously for $\wilson[NP]{i}$. } 
...
with \christoph{$\mbox{dim} \geq 8$}. These results imply that for SM values of the effective
couplings
\begin{align}
  F_H^\ell(q^2) \big|_{\rm SM} & \propto \frac{m_\ell^2}{q^2} 
    \christoph{\bigg/ \Gamma_\ell} \,, &
  A_{\rm FB}^\ell \big|_{\rm SM} & = 0 \,.
\end{align}
Hence, for $\ell = e,\, \mu$ both observables are quasi-null tests. 
The flat term $F_H^\ell(q^2)|_{\rm SM}$ is strongly suppressed \christoph{by small 
lepton masses for the considered kinematical region $1 \leq q^2 \leq 22$ GeV$^2$}
\cite{Bobeth:2007dw, Bobeth:2011nj, Bouchard:2013eph}.  
Nonzero values of $A_{\rm FB}^\ell|_{\rm SM}$ can be induced by higher order QED 
corrections, which will modify the simple $\cos\theta_\ell$ dependence of the angular
distribution~\refeq{eq:ang-distr}, however, currently there is no solid estimate
available for this source of SM background\footnote{The \christoph{logarithmically
enhanced NLO QED corrections}
  to $B\to X_s\bar\ell\ell$ \cite{Huber:2015sra} might provide an estimate of
  corresponding effects in $B\to K \bar\ell\ell$.}. \todo{David's comment?!}
...
\begin{figure*}
  ...
  \end{center}
  \caption{ The sensitivity of to the tensor coupling $\mbox{Re}(\wilson{T})$
    of\christoph{: $F_H^\mu$} (first row) and \christoph{${\cal B}_\mu$} (second row) 
    in $B^+ \to K^+ \bar\mu\mu$
    \christoph{as well as} \christoph{${\cal B}_\mu$} (third row), $(J_{1c} + J_{2c})$ 
    (fourth row), and $(J_{1s}-3 J_{2s})$ (fifth row) in $B^0 \to K^{*0} \bar\mu\mu$ 
    integrated over low- and high-$q^2$ bins. In the last two rows, the observables 
    are rescaled by the
...
\wilson{10,10'})$ are suppressed by $m_\ell/\sqrt{q^2}$. \christoph{They become numerically
relevant in cases $\wilson{T} \ll \wilson{7,7',9,9'}$ or $\wilson{P,P'}
\ll\wilson{10,10'}$, where the smallness of Wilson coefficient is of the
same level as the suppression factor $m_\ell/\sqrt{q^2}$ of the large vectorial
SM Wilson coefficients $\wilson[SM]{9,10}$, which implies however no large
enhancement of $F_H^\ell$ over the SM prediction.}
...
\to K$ observable is attained by \christoph{$F_H^\mu$} at high $q^2$ due to a partial
cancellation of form factors \cite{Bobeth:2012vn}. If the experimental
uncertainty could be reduced further, \christoph{$F_H^\mu$} would give a very strong
constraint on a simultaneous negative shift in $\wilson[NP]{9}$ and
\christoph{$\wilson{T}$}. The prediction of \christoph{${\cal B}(B\to K \bar\mu\mu)$} is
essentially insensitive to $\wilson{T}$ but sensitive to \christoph{$\wilson[NP]{9}$}. A
stronger impact on global fits, however, would \christoph{require a} reduced theory
uncertainty.

The observable \christoph{${\cal B}(B\to K^* \bar\mu\mu)$} shows moderate dependence on
...
strong dependence on $\wilson{T}$ appears at low $q^2$.  Overall, \christoph{$F_H^\mu$},
$(J_{1c}+J_{2c})$ at \christoph{high} $q^2$, and $(J_{1s}-3J_{2s})$ are sensitive
to $\wilson{T}$ and theoretically very clean around $\wilson{T}=0$.

From the available measurements, \christoph{$F_H^\mu$} at high $q^2$ currently provides the
...
Eventually we also explore the effect of interference with \christoph{NP contributions}
...
imaginary parts of $\wilson{9,\,9',\,10,\,10'}$. The experimental \christoph{input} of all 
...
the enhanced sensitivity of both observables to \christoph{these} couplings compared to
...
For each coupling \christoph{that} we vary in a fit, we remain as general as possible, treat it
...
interference effects with new physics in vector couplings \christoph{$\wilson{9,9',10,10'}$}. Finally, we conclude
...
\christoph{$J_{6c}$, $(J_{1c}+J_{2c})$ and $(J_{1s} - 3J_{2s}$)}
...
In a scenario with only \christoph{complex-valued tensor couplings} $\wilson{T,\, T5}$, the
...
\christoph{ones}~\cite{Horgan:2013pva}. Since tensor couplings contribute constructively
...
which we attribute to \christoph{the} cumulative effect of $\wilson{S,S',P,P'} \ne 0$ in
...
\begin{figure*}
  ...
  \caption{ The constraints on complex-valued $\wilson{S,\,S',\,P,\,P'}$ from
    only $F_H^\mu$~\christoph{[grey dotted]}, only $\overline{\cal B}(B_s \to
    \bar\mu\mu)$ \christoph{[blue dashed]}, and the combination with all other
    data in \reftab{tab:observables} as well as nonzero
    $\wilson[]{9,\,9',\,10,\, 10'}$ \christoph{[red solid]} at 68\% (darker)
    and 95\% (lighter) probability. The constraints on Re$(\wilson{P} \pm
    \wilson{P'})$ are identical to Re$(\wilson{S} \pm \wilson{S'})$, apart from
...
In the following we consider a \christoph{scenario consisting
of the SM augmented} by dimension six operators that respect the SM gauge group and are
...
order of the $W$-boson \christoph{mass in~\cite{D'Ambrosio:2002ex}} for vector
...
$\Lambda_{\rm NP}$ is the high-energy scale at which new physics \christoph{
is decoupled}.
...
\christoph{$B \to K \bar\mu\mu$ and $B_s \to \bar\mu\mu$}. In the second fit, we allow
$\wilson[NP]{10, 10'} \ne 0$ and further include all \christoph{$B \to K^* \bar\mu\mu$}
  ...
  \begin{center}
    ...
    \caption{ 68\% and 95\% contours of the 2D-marginalized distributions of
      ...
      superimpose the corresponding contours from using only $\overline{\cal
        B}(B_s\to \bar\mu\mu)$ and all \christoph{$B \to K \bar\mu\mu$} constraints
...
Now we discuss what the \christoph{fits tell} us about likely values of observables that
...
\todo{Fred should clarify with Danny on their paper.}
...
\christoph{$q^2 \in [1.1,\, 6]$} and $[15,\, 19]$ GeV$^2$ bins.
...
\pi)$ or \christoph{$m_b m_\ell/m_W^2$}. These factors should be compared to the potential
...
\wilson{P} + \wilson{S} \times \wilson{T5}$, \christoph{$m_{\ell}/\sqrt{q^2} \times
...
(tensor) couplings is largest. But even then, it \christoph{seems that the} experimental
...
value in the range $[-1,\, 1]$ is \christoph{allowed} whereas \christoph{the SM 
and the scenario with only tensor couplings predicts precisely one \cite{DeBruyn:2012wk}}; 
cf. the last row in \reftab{tab:postdictions}. Hence any deviation from one would
...
couplings that mediate $b\to s\bar\mu\mu$ transitions\christoph{. They are based on}
the latest
...
\christoph{Both,} $F_H^\mu$ and $A_{\rm
...
\begin{enumerate}
  \item imposes by itself \christoph{constraints} on tensor couplings $|\wilson{T,\,T5}|$ such
  ...
  data from $B\to K^* \bar\mu\mu$ and lattice \christoph{predictions of} $B\to K^*$ form factors, the
  bounds are lowered to 0.33 (0.43), even in the presence of nonstandard
  contributions in vector couplings.
  \item for the first time allows to \christoph{simultaneously} bound all four scalar couplings
  \christoph{$\wilson{i = S,S',P,P'}$} due to it's complementarity to
...
realization of the $SU(2)_L \otimes U(1)_Y$ electroweak symmetry group.

Our study of \christoph{yet unmeasured} angular observables $J_{6c}$, $(J_{1c} + J_{2c})$,
...
reach of the LHCb analysis of the full \christoph{Run} I data set. Our bounds on scalar
...
$A_{\Delta\Gamma}(B_s\to \bar\mu\mu)$ of \christoph{nonzero scalar couplings. 
Currently, it can take on any value in the range $[-1,\, 1]$.}
...
\christoph{Contrary to \cite{Beaujean:2013soa}, we do not choose a log-gamma distribution
...
relations at \christoph{low and high $q^2$} for the tensor and scalar $B\to K$ and $B\to
K^*$ form factors.  

\christoph{Therefore, we need} additional nuisance parameters for the
...
$B$- and $K^*$-meson masses. The \christoph{pole} masses $M_{R,i}$ are set to
...
covariance. \todo{Really? We managed with 2, but not with 3?!} We then contacted the authors of~\cite{Horgan:2015vla} and obtained
...
and standard \christoph{deviations} of that three-parameter fit are given in
...
parameters $\vecth$ \christoph{and} given as
\begin{align}
  \label{eq:predictive}
