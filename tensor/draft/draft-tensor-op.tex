...
for vector couplings $\wilson{7,\,9,\,10}$. The matching for tensor
...
much upper bounds on $|\wilson{S,\, S'}|$ found in \cite{Alonso:2014csa}. Since we
consider here complex-valued couplings the allowed regions are circles rather
than rings as for the case of real-valued couplings. The result of the simultaneous
fit of complex-valued $\wilson{S,\,S',\,10,\,10'}$ is shown in
\reffig{fig:scalar:SM-EFT}.
...
$\wilson{S,S',P,P',T,T5}$ in combination with complex-valued vector couplings
$\wilson{9,\,9',\,10,\, 10'}$ --- i.e. the combination of \refsec{sec:tensor}
and \refsec{sec:scalar} with same prior ranges --- does not yield very different
bounds. The changes are only small and in fact the bounds tend to be even more
stringent, because tensor and scalar couplings can contribute to $F_H^{\mu}$
only constructively --- see \refeq{eq:FH-dependence}.
As before branching-ratio measurements of $B\to K^{*}\bar\mu\mu$ help to strengthen
the constraints on $\wilson{T,\, T5}$. This demonstrates that even in the case
...
Eventually, we present the yet allowed ranges due to non-standard tensor
...
for the various model-independent scenarios, allowing for interference with
non-zero vector couplings, in \refsec{sec:tensor}, \refsec{sec:scalar}
...
\begin{align}
  ...
  \right]
\end{align}
is numerically of the order ${\cal O}(10^{-7})$ in the SM
\begin{equation}
\begin{aligned}
  \langle {\rm d}\BR/{\rm d}q^2 \rangle_{[1.1,\, 6]} & =
  \left(1.86_{-0.44}^{+0.53} \right) \cdot 10^{-7} \,,
\\
  \langle {\rm d}\BR/{\rm d}q^2 \rangle_{[15,\, 19]} & =
  \left(1.91_{-0.16}^{+0.16} \right) \cdot 10^{-7} \,,
\end{aligned}
\end{equation}
when integrated over low- and high-$q^2$ regions. \todo{Elaborate on larger
theory uncertainties at low-$q^2$, due to subleading corrections.
Form factor uncertainties are small due to correlations, would be larger
when neglecting them.}

The angular observable $J_{6c}$
...
($d \geq 8$) $|\Delta B| = |\Delta S| = 1$ operators, leading to parametrical
suppression of $\alpha_e/(4 \pi)$ or $(m_b/m_W)^2$, respectively. Such a
suppression must be compared to potential suppression present for tensor
...
constraints on tensor and scalar couplings.

Beyond the SM, $J_{6c}$ can become of order ${\cal O}(10^{-9})$ in scenarios
involving tensor couplings and only ${\cal O}(10^{-10})$ in their absence.
...
tensor couplings. In this case enhancement by two orders of magnitude are
possible up to the ${\cal O}(10^{-8})$ at high-$q^2$ and also at low-$q^2$ in
...
experimental precision of $20\%$ of the branching ratio measurements at LHCb
\cite{Aaij:2013iag} with only 1 fb$^{-1}$, corresponding to the ${\cal O}(10^{-8})$, one can
indeed hope for some sensitivity to such large effects in $(J_{1c} + J_{2c})$
and $(J_{1s} - 3\, J_{2s})$ for the not yet published 3 fb$^{-1}$ data set.
\todo{Would be a nice place to cite method of moments paper?}
