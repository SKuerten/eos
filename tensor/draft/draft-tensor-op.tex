...
\\
  ...
  \caption{
    The sensitivity of $F_H^\mu$ [upper row: 1, 2], ${\cal B}(B^+ \to K^+ 
...
Important additional constraints on scalar couplings come from
the branching ratio of $B_s \to \bar\mu\mu$ as given in \refeq{eq:Br-Bsmumu}.
It provides the most stringent constraints on the moduli $|\wilson{S} - \wilson{S'}|$
...
We will also make use of the branching ratio of $B\to K^* \bar\mu\mu$ as it
allows us to $\ldots$
\todo{$A_{\rm CP}$ included.}

\todo{Find a place to cite \cite{Becirevic:2012fy}, perhaps in section on results
comparing our bounds to theirs in eq.(49): $|\wilson{S,\,S'}|Ê\lesssim 0.7$ and
$|\wilson{P,\,P'}|Ê\lesssim 1.0$ - what about \cite{Altmannshofer:2012az}
and \cite{Hurth:2012jn, Hurth:2013ssa} (this is only MFV, i.e. only one operator)?}.

In our fit of the effective couplings $\wilson{i}(\mu_b)$, we do not neglect
kinematically suppressed terms in observables, neither in the prediction of
yet not measured observables. The experimental input of these observables is 
listed in \reftab{tab:observables} together with input for the $B\to K^{(*)}$
form factors. More details on the latter can be found in \refapp{app:theory:inputs}.
...
\begin{table*}
  ...
  \hline \hline
  $\mbox{Re}\,\wilson{T}$
  & $\quad$ $[-0.35,\, 0.20]$ ($[-0.55,\, 0.40]$) $\quad$
  & $\quad$ $[-0.20,\, 0.15]$ ($[-0.35,\, 0.30]$) $\quad$
  & $\quad$ $[-0.20,\, 0.20]$ ($[-0.40,\, 0.35]$) $\quad$
  \\
  $\mbox{Im}\,\wilson{T}$
  & $[-0.25,\, 0.25]$ ($[-0.45,\, 0.45]$)
  & $[-0.15,\, 0.15]$ ($[-0.30,\, 0.30]$)
...
The according 65\% (95\%) 1D-marginalized probability intervals are listed in
...
by allowing uniform priors for $\mbox{Re, Im}(\wilson[NP]{i}) \in[-3,\, 3]$,
...
bounds $|\wilson{T,\,T5}| < 0.5\,(0.8)$ at 68\% (95\%) probability that are
...
(B_s \to \bar\mu\mu)$ on $(\wilson{i} - \wilson{i'})$ is about a factor five
...
%  \includegraphics[width=.32\textwidth]{plots/pdf/res-PPp.pdf}
  ...
  \caption{
    ...
    $\wilson{10} \in [-1,\, 9]$, $\wilson{10'} \in [-5,\,5]$ (red) 
...
%}
  ...
  \caption{
     ...
     $\wilson{10,\,10'}$ with uniform prior $\mbox{Re}\,\wilson[NP]{10} \in [-1,\, 9]$,
...
\begin{enumerate}
\item imposes by itself constraints on tensorial couplings $|{\cal C}_{T,\,T5}^\mu|
  ...
  $B\to K^*$ form factors they become tighter $|{\cal C}_{T,\,T5}^\mu| < 0.30\, 
...
\item allows for the first time to bound all four scalar and pseudo-scalar 
  ...
  $|\wilson{i} + \wilson{i'}| < 0.4\, (0.7)$ and $|\wilson{i} - \wilson{i'}| < 
  0.1\, (0.2)$ for $i = S,P$. Currently, the bounds from $F_H^\mu$ are not as
  strong as those from $\overline{\cal B}(B_s \to \bar\mu\mu)$ by a factor four.
  Future measurements at LHCb and Belle II might change this, especially for 
  $\ell = e$. 
\item SM-EFT $\ldots$
\end{enumerate}
In all cases these constraints are weakened only slightly in the presence of
...
are negligible. \todo{Check for updated values for $y_s$ of $B_s$?} Further, we 
...
The most important change in the treatment of $B\to K$ form factors is the consistent
use of correlations available from lattice \cite{Bouchard:2013eph} for all three form
factors  $f_{+,T,0}$ in the construction of their prior. We use the $z$-parameterization
\cite{Khodjamirian:2010vf}, such that only five nuisance parameters --- see
\reftab{tab:hadronic:nuisance} --- arise due to the kinematical relation 
$f_0(0) = f_+(0)$. 
...
For $B\to K$ we have modified the prior of the nuisance parameter $f_+(0)$ of
the $f_+$ form factor parametrization w.r.t our previous analysis but kept the
slope parameter $b_1^+$ as is. This change accounts for both LCSR results
\cite{Khodjamirian:2010vf,Ball:2004ye} that use the same approach of
$B$-interpolating currents and on-shell $K$-mesons. As a result the prior on
$f_+(0)$ is wider and the tension between the SM prediction of the $B\to K
\ell\bar\ell$ branching ratio at $q^2 \in[1, 6]$~GeV$^2$~\cite{Khodjamirian:2012rm}
and the LHCb measurement \cite{Aaij:2012vr} is
reduced. Moreover, the recent lattice predictions \cite{Bouchard:2013eph} of the
form factor $f_+$ at high $q^2$ are included in our analysis as part of the
likelihood for technical reasons. For this purpose we reproduced lattice
predictions at three values
of $q^2 = 17,\, 20,\, 23$~GeV$^2$ as well as their correlation matrix based on
the parametrization given in \cite{Bouchard:2013eph}, see
\reftab{tab:btok-lattice}.  (The $q^2$ values and number of points are chosen
such that the correlation of neighboring points does not exceed $80\%$). This
constraint is included in the likelihood by means of a multivariate Gaussian.
