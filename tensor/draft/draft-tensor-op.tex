...
Now we present the yet allowed ranges due to non-standard tensor and scalar
couplings of the so far unmeasured angular observables $J_{6c}$, $(J_{1s} - 3\,
J_{2s})$ and $(J_{1c} + J_{2c})$ in $B\to K^*(\to K\pi)\bar\ell\ell$ in order to
explore the potential of their future measurements in fits of the kind pursued
in this work.  The 68\% probability intervals are collected in
\reftab{tab:postdictions} for the various model-independent scenarios, allowing
for interference with non-zero vector couplings, in \refsec{sec:tensor},
\refsec{sec:scalar} and \refsec{sec:tensor:scalar} for $q^2$-integrated
quantities in the low- and high-$q^2$ bins $q^2\in [1.1,\, 6]$ GeV$^2$ and
$q^2\in [15,\, 19]$ GeV$^2$, respectively. Their posterior distributions have
been obtained from the posterior samples as explained in~\ref{app:mc}.
...
\label{app:mc}
\todo{reference in main text} The marginalization of the posterior is performed
