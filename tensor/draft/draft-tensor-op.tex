...
\cite{Bobeth:2007dw, Bobeth:2011nj, Bouchard:2013eph}.  Nonzero values of
...
We also perform a fit with nonzero $\wilson[NP]{9,\,10}$ \fred{Is this
...
\begin{figure*}
  ...
  \caption{ The constraints on complex-valued $\wilson{S,\,S',\,P,\,P'}$ from
    ...
    data in \reftab{tab:observables} as well as nonzero
...
\begin{table*}
  ...
  \caption{
     ...
     The posterior predictive 68\% probability intervals of not-yet-measured angular observables in
%     $B^0\to K^{*0}\bar\mu\mu$ integrated over $q^2$ bins at low- and high-$q^2$
     for several new-physics scenarios given all the considered experimental
     constraints. The corresponding values for the SM (prior predictive) are given, too, where ``$\simeq 0$''
     indicates zero in the considered
     approximation, see text for details. \checked{}
   }
\end{table*}

Now we discuss what the fits tells us about likely values of observables that
...
(\refsec{sec:tensor}), only $\wilson{S^{(')},P^{(')}}$ (\refsec{sec:scalar}),
and finally both tensor and scalar couplings (\refsec{sec:tensor:scalar}).

We rescale the observables by the $B^0$-meson life time $\tau_{B^0} =
1.519$~ps~\cite{Agashe:2014kda} to be able to judge if experiments can measure
them in the near future.  At low $q^2$ the posterior predictive ranges are
${\cal O}(10^{-9})$ for $(J_{1s} - 3\, J_{2s})$ and $(J_{1c} + J_{2c})$, getting
suppressed down to ${\cal O}(10^{-10})$ at high $q^2$ due to the
$m_\ell/\sqrt{q^2}$ dependence. In order to compare it to the typical magnitude
of vector couplings, note that for example the branching ratio of $B\to
K^*\bar\ell\ell$
\begin{align}
...
\pi)$ or $(m_b/m_W)^2$. These factors should be compared to the potential
...
measurements, in combination with theory uncertainties, do not yet impose too
...
\wilson{P} + \wilson{S} \times \wilson{T5}$, $\frac{m_{\ell}}{\sqrt{q^2}}
(\mbox{scalar}) \times (\mbox{vector})$, and $\frac{m_{\ell}}{\sqrt{q^2}}
...
(tensor) couplings is largest. But even then, it seems the experimental
...
For the CP asymmetry $A_{\Delta\Gamma}(B_s\to \bar\mu\mu)$ induced by the
nonvanishing width of the $B_s$ meson (cf. \refapp{app:BKstarellell}), we find a
rather uniform distribution in scenarios with nonzero scalar couplings. So any
value in the range $[-1,\, 1]$ is OK whereas we predict precisely one in the SM
and the scenario with only tensor couplings; cf.  the last row in
\reftab{tab:postdictions}. Hence any deviation from one would unambiguously hint
at the presence of scalar operators.
...
\item imposes by itself constraints on tensor couplings $|\wilson{T,\,T5}|
  ...
  (0.43)$, even in the presence of nonstandard contributions in vector
  couplings.
...
$A_{\Delta\Gamma}(B_s\to \bar\mu\mu)$ induced by nonvanishing width of the
...
of form factors from different nonperturbative approaches, namely LCSR's at
