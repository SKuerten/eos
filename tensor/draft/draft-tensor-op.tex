...
\begin{abstract}
  ...
  term, $F_H^\ell$, that are strongly suppressed in the standard model. In both,
  nonstandard tensor and scalar contributions are enhanced with respect
  to vector ones. We use latest experimental results for $\ell = \mu$ in
  combination with $B_s\to \bar\mu\mu$ to derive stronger model-independent
  constraints on tensor and scalar effective couplings and determine their
  impact on not yet measured angular observables in
  $B\to K^*\bar\mu\mu$, which feature the same suppression of vector couplings.
  Most notably, the measurement of $F_H^\mu$ provides
