...
class Translator:
    """
    Translate EOS names into LaTex names.
    """

    # "static" member
    names = {# scan parameters
                  "Abs{c7}" :r"$\left|C_{7}\right|$",
                  "Abs{c9}" :r"$\left|C_{9}\right|$",
                  "Abs{c10}":r"$\left|C_{10}\right|$",
                  "Arg{c7}" :r"$\arg C_{7}$",
                  "Arg{c9}" :r"$\arg C_{9}$",
                  "Arg{c10}":r"$\arg C_{10}$",
                  "Abs{c7'}" :r"$\left|C_{7}^{'}\right|$",
                  ...
                  "Re{c9}":"$%s$" % wilson(9),
                  "Re{c10}":"$%s$"% wilson(10),
                  "Re{c7'}":r"$\Re\left(C_{7}^{'}\right)$",
                  "Re{c9'}":r"$\Re\left(C_{9}^{'}\right)$",
                  "Re{c10'}":r"$\Re\left(C_{10}^{'}\right)$",
                  "Im{c7}":r"$\Im\left(C_{7}\right)$",
                  ...
                  "B->K::F^p(0)@KMPW2010":r"$FF(KMPW2010):F^p(0)$",
                  "B->K::b^p_1@KMPW2010":r"$FF(KMPW2010):b^p_1$",
    ...
    @staticmethod
    def to_tex(eos_name):
        if eos_name in Translator.names:
            return Translator.names[eos_name]
        else:
            return eos_name
...
def factory(cmd_line=None):
    ...
    parser.add_argument('--no-nuisance-vs-nuisance', help="Don't produce 2D plots if both are nuisance parameters",action='store_true')
